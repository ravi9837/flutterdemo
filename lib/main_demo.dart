...
import 'package:flutterdemo/Model/questionsModelForDemo.dart';
...
class MyHomePageState extends State<MyHomePage> {
  ...
  incrementForMainFlow() {
    ...
    if (masterFlow[currentMainIndex].kind == 2) {
      ///IF ANSWER IS YES
    ...
    ///FOR START
    if (masterFlow[currentMainIndex].kind == 4 && masterFlow[currentMainIndex].next.isNotEmpty) {
      setState(() {
        currentMainIndex = getMainIndexById(masterFlow[currentMainIndex].next[0].destElementId);
        masterFlow[currentMainIndex].previousId = previousMainId;
      });
  ...
  incrementForSubFlow() {
    ...
    if (subFlow[currentIndex].kind == 2) {
      if(skipping == false){
        setState(() {
          previousSubId = subFlow[currentIndex].id;
        });
    ...
    ///FOR START
    if (subFlow[currentIndex].kind == 4 && subFlow[currentIndex].next.isNotEmpty) {
      if(skipping == false){
        setState(() {
          previousSubId = subFlow[currentIndex].id;
        });
    ...
    ///FOR END
    if (subFlow[currentIndex].kind == 4 && subFlow[currentIndex].next.isEmpty ) {
  ...
  int getMainIndexById(String id) {
    ...
    return -1; // Return -1 if element with matching id is not found
  }
  ...
  bool checkCondition(){
    ...
    for (int i=0 ; i<subFlow[currentIndex].ageGroupList.length ; i++){
      print("in check condition1");
      if(subFlow[currentIndex].ageGroupList[i].start <= age && age <= subFlow[currentIndex].ageGroupList[i].end){
    ...
    for (int i=0 ; i<subFlow[currentIndex].genderGroupList.length ; i++){
      print("in check condition4");
      if(subFlow[currentIndex].genderGroupList[i]==gender){
